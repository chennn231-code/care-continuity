...
export const OCCURRENCE_TYPES = ['DAILY', 'WEEKLY', 'ONCE', 'AS_NEEDED'] as const;
...
export const OCCURRENCE_TYPE_LABELS: Record<OccurrenceType, string> = { DAILY: '每天', WEEKLY: '每週特定日', ONCE: '特定日期', AS_NEEDED: '視需要' };
...
export function normalizeTaskInput(values: TaskFormValues): NormalizedTaskInput {
  ...
  const times = values.occurrenceType === 'DAILY' || values.occurrenceType === 'WEEKLY'
    ? normalizeScheduledTimes(values.scheduledTimes)
    : [];
  ...
  else if (values.occurrenceType === 'ONCE') {
    if (!isValidCalendarDate(values.onceDate)) throw new TaskValidationError('請選擇有效的特定日期');
    if (!isValidScheduledTime(values.onceTime)) throw new TaskValidationError('時間必須是 00:00–23:59 的 24 小時制格式');
    occurrence_pattern = { type: 'ONCE', date: values.onceDate, scheduled_time: values.onceTime };
  }
...
export function describeOccurrence(pattern: OccurrencePattern) {
  ...
  if (pattern.type === 'ONCE') return `特定日期｜${pattern.date.replaceAll('-', '/')} ${pattern.scheduled_time}`;
