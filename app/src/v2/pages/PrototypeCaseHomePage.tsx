...
export function PrototypeCaseHomePage() {
  ...
  const unresolved = state.questions.filter((item) => item.caseId === caseId && item.status !== 'RESOLVED');
  ...
  return (
    <section className="v2-page">
      ...
      <section className="v2-new-summary" aria-labelledby="new-summary-heading"><div><span>上次查看後</span><strong>{visibleTimelineEntries(state, state.activeRole, caseId).length}</strong><span>筆新變化</span></div><div><h2 id="new-summary-heading">先看發生了什麼</h2><p>時間軸成功載入時才會更新本人的查看位置，本 Prototype 不建立公開已讀名單</p></div><Link className="secondary-button" to={`/v2/prototype/cases/${caseId}/timeline`}>查看照顧變化</Link></section>
