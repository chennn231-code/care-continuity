...
export function PrototypeCaseHomePage() {
  ...
  return (
    <section className="v2-page">
      <header className="v2-page-heading v2-heading-actions"><div><p className="eyebrow">虛構展示個案</p><h1>林奶奶的照顧協作</h1><p>目前個案關係：家庭成員｜有效 grant path：家庭共同照顧</p>{state.activeRole !== 'FAMILY' && <small>目前正在使用 Prototype 權限預覽：{DEMO_ROLE_LABELS[state.activeRole]}</small>}</div><Link className="primary-button" to="/v2/prototype/cases/demo-case/updates/new">新增照顧變化</Link></header>
