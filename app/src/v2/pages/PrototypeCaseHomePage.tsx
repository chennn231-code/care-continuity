...
import { caseActivityResponseForCurrentActor, currentCaseActorContext, visibleActionsForCurrentActor, visibleCareCircleForCurrentActor, visibleQuestionsForCurrentActor } from '../state/caseCollaborationSelectors';
import { StatusPill } from '../components/PrototypeShell';
import { canCreateProfessionalRecord } from '../state/professionalRecordState';
import { demoActionOperationAvailability } from '../state/prototypeState';
...
export function PrototypeCaseHomePage() {
  ...
  const activityResponse = caseActivityResponseForCurrentActor(state, caseId);
  const activities = activityResponse.items;
  const latest = activities[0];
  const visibleActions = visibleActionsForCurrentActor(state, caseId);
  const waiting = visibleActions.filter((item) => item.status === 'PENDING_ACCEPTANCE' && demoActionOperationAvailability(state, item.id, 'ACCEPTED').allowed);
  const active = visibleActions.filter((item) =>
    (item.status === 'ACCEPTED' && demoActionOperationAvailability(state, item.id, 'IN_PROGRESS').allowed)
    || (item.status === 'IN_PROGRESS' && demoActionOperationAvailability(state, item.id, 'COMPLETED').allowed));
  ...
  return (
    <section className="v2-page">
      ...
      <div className="v2-dashboard-grid">
        <section className="v2-card"><div className="v2-section-title"><h2>最近一次可見活動</h2>{latest && <StatusPill tone="active">有活動</StatusPill>}</div>{latest ? <><p className="v2-card-copy">{latest.summary}</p><small>{latest.actorLabel}｜{new Date(latest.displayTimestamp).toLocaleString('zh-TW')}</small></> : <p>目前沒有可見活動；這不代表沒有其他未授權資料。</p>}</section>
