...
import { allowedUpdateAssigneeRoles, allowedUpdateScopes, updateGrantPaths } from '../state/caseCollaborationSelectors';
...
export function PrototypeNewUpdatePage() {
  ...
  const submit = (event: FormEvent) => {
    ...
    if (!activePath || !scopeOptions.includes(form.sharingScope) || (form.needsAction && !assigneeOptions.includes(form.assigneeRole ?? state.activeRole))) { setError('目前授權路徑不允許這組身分、目的、分享範圍或負責人'); return; }
    addUpdate(form);
  ...
  return (
    <section className="v2-page v2-form-page">
      ...
      <form className="v2-card form-stack" onSubmit={submit}>
        ...
        {form.needsAction && <div className="v2-followup-fields"><label>指定虛構負責人<select value={form.assigneeRole} onChange={(event) => update('assigneeRole', event.target.value as DemoRole)}>{assigneeOptions.map((role) => <option value={role} key={role}>{DEMO_ROLE_LABELS[role]}</option>)}</select></label><label>期限<input type="datetime-local" value={form.dueAt} onChange={(event) => update('dueAt', event.target.value)} /></label><p>建立後會先顯示「等待接受」，不會假設對方已同意處理</p></div>}
