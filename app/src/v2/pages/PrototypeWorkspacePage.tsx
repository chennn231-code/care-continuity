...
import { effectiveInvitationStatus, managerReassignmentItems, searchVisibleWorkspaceCases, waitingStartWorkspaceCases } from '../state/invitationWorkspaceState';
import { usePrototype } from '../state/PrototypeProvider';

export function PrototypeWorkspacePage() {
  const { state, removeCaseAccess, resendInvitation, setRole, toggleCaseTag } = usePrototype();
  ...
  const pendingInvitations = state.invitations.filter((item) => effectiveInvitationStatus(item) === 'INVITED');
  const unavailableInvitations = state.invitations.filter((item) => effectiveInvitationStatus(item) !== 'INVITED' && item.status !== 'ACCEPTED');
