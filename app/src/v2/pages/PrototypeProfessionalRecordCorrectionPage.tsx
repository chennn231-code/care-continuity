...
export function PrototypeProfessionalRecordCorrectionPage() {
  ...
  if (state.activeRole !== 'NURSE' || !record || record.caseId !== caseId || !professionalRecordAccess(state, recordId)?.canViewFull || !canCreateProfessionalRecord(state, caseId)) return <section className="v2-page v2-access-denied-page"><h1>目前無法更正此紀錄</h1><Link to={`/v2/prototype/cases/${caseId}/records`}>返回專業紀錄</Link></section>;
