...
export function PrototypeProfessionalRecordsPage() {
  ...
  const canCreate = canCreateProfessionalRecord(state, caseId);
  return (
    <section className="v2-page">
      ...
      {!canCreate && <section className="v2-card v2-record-acting-context"><h2>目前不是專業紀錄操作情境</h2><p>建立紀錄必須使用已驗證且具有效個案關係的具名參與者，不能與家庭權限合併使用。</p>{state.members.filter((member) => member.caseId === caseId && member.status === 'ACTIVE').map((member) => <button className="secondary-button" type="button" onClick={() => selectActor(caseId, member.participant, member.role)} key={member.participant.membershipId}>以 {member.name} 檢視</button>)}</section>}
