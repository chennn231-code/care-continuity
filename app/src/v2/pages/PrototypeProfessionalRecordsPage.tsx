...
import { canCreateProfessionalRecord, familyProfessionalRecordProjection, latestProfessionalRecordVersions, PROFESSIONAL_RECORD_DEMO_LABEL } from '../state/professionalRecordState';
    ...
    <section className="v2-page">
      ...
      <div className="v2-record-list">
        {state.activeRole === 'FAMILY'
          ? familyRecords.map((record) => <article className="v2-card" key={record.recordId}><div className="v2-section-title"><div><span className="v2-fake-label">家屬獲授權內容</span><h2>專業照顧摘要</h2></div>{record.wasCorrected && <strong>曾經更正</strong>}</div><p>{record.objectiveObservation}</p><dl className="v2-meta-grid"><div><dt>作者與身分</dt><dd>{record.authorLabel}</dd></div><div><dt>服務目的</dt><dd>{record.purpose}</dd></div></dl><Link className="text-button" to={`/v2/prototype/cases/${caseId}/records/${record.recordId}`}>查看獲授權摘要</Link></article>)
