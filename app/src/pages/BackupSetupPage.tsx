...
export function BackupSetupPage() {
  ...
  return <main className="task-setup-layout assignment-setup-layout">
    ...
    {eligibleSources.length === 0 && <section className="task-form-card"><h2>還沒有可設定的備援來源</h2><p>主要照顧者本人不能成為自己的備援請先加入其他家人、親友或服務來源</p><button className="primary-button" onClick={() => navigate('/setup/sources')}>前往照顧來源</button></section>}
    ...
    {editingTask && <section className="task-form-card assignment-editor"><p className="eyebrow">{editingBackupId ? '修改備援安排' : '新增備援安排'}</p><h2>{editingTask.title}</h2><form className="form-stack task-form" onSubmit={submit}>
      ...
      {form.confirmationStatus === 'CONFIRMED_WITH_LIMITS' && pattern && pattern.type !== 'AS_NEEDED' && pattern.type !== 'ONCE' && <fieldset><legend>可接手時間</legend><div className="choice-grid three-columns">{pattern.scheduled_times.map((time) => <label className="choice-card" key={time}><input type="checkbox" checked={form.scheduledTimes.includes(time)} onChange={() => toggleTime(time)} />{time}</label>)}</div></fieldset>}
