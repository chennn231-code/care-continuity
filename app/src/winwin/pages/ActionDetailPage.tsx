...
type Decision = 'ACCEPT_ACTION' | 'DECLINE_ACTION';
type DecisionState = 'idle' | 'submitting' | 'uncertain' | 'definitelyNotCommitted' | 'conflict' | 'committedPendingRefresh';
type Intent = Readonly<{ family: Decision; operationKey: OperationKey; input: ActionMutationInput }>;
type DecisionOwner = Intent | Readonly<{ acquiring: true; family: Decision }>;
...
export function ActionDetailPage({ caseId, actionId }: Readonly<{ caseId: string; actionId: string }>) {
  ...
  const begin = (family: Decision) => {
    if (!detail || owner.current) return;
    if (detail.lifecycleState !== 'ASSIGNED' || !detail.allowedOperations[family]) return;
    owner.current = { acquiring: true, family };
    try {
      const intent: Intent = {
        family,
        operationKey: `${family === 'ACCEPT_ACTION' ? 'accept-action' : 'decline-action'}:${globalThis.crypto.randomUUID()}`,
    ...
    try {
      ...
      if (retry && status.outcome === 'DEFINITELY_NOT_COMMITTED') {
        ...
        if (current.result !== 'SUCCESS'
          ...
          || current.data.lifecycleState !== 'ASSIGNED'
  ...
  return <article className="winwin-action-detail" aria-labelledby="action-detail-heading" aria-busy={busy}>
    ...
    {decisionState === 'uncertain' && <div className="winwin-mutation-notice" role="alert"><p>目前無法確認是否已成功更新接手狀態。系統不會自動再次送出。</p><button type="button" disabled={lookupBusy} onClick={() => void lookup(false)}>{lookupBusy ? '正在查詢…' : '查詢更新狀態'}</button></div>}
    {decisionState === 'definitelyNotCommitted' && <div className="winwin-mutation-notice" role="alert"><p>接手狀態尚未更新。重新送出前會先確認目前指派。</p><button type="button" disabled={lookupBusy} onClick={() => void lookup(true)}>{lookupBusy ? '正在確認…' : '確認目前狀態並重試'}</button></div>}
    {decisionState === 'conflict' && <p className="winwin-mutation-notice" role="alert">這次更新發生衝突，系統沒有再次送出。請重新整理後再決定。</p>}
    {decisionState === 'committedPendingRefresh' && <div className="winwin-mutation-notice" role="status"><p>變更已送出，但最新狀態尚未重新載入完成。系統不會再次送出接手決定。</p><button type="button" disabled={lookupBusy} onClick={() => void retryCommittedRefresh()}>{lookupBusy ? '正在重新載入…' : '重新載入最新狀態'}</button></div>}
